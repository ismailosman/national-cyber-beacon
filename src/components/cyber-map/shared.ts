...
const COUNTRY_CITIES: Record<string, { lat: number; lng: number }[]> = {
  'USA': US_CITIES, 'United States': US_CITIES, 'United States of America': US_CITIES,
...
export function jitterCoords(lat: number, lng: number, seed: string, country?: string): { lat: number; lng: number } {
  ...
  // City pool: snap to a real city with tiny jitter
  if (country && COUNTRY_CITIES[country]) {
    const cities = COUNTRY_CITIES[country];
    const idx = Math.abs(hashString(seed)) % cities.length;
    const city = cities[idx];
    return { lat: city.lat + (rand() - 0.5) * 0.6, lng: city.lng + (rand() - 0.5) * 0.6 };
  }
  // Large country fallback
